    ...
    public function storePrediction(Request $request)
    {
        try {
            $validated = $request->validate([
                ...
                'imc' => 'required|numeric|min:15|max:40',
                ...
                'diagnostico' => 'required|integer|in:0,1',
                'medico_encargado' => 'required|string|max:255',
            ]);

            // Check if a prediction already exists for this user
            $existingPrediction = Prediction::where('user_id', $validated['user_id'])->first();

            if ($existingPrediction) {
                // Update existing prediction
                $existingPrediction->update($validated);
                return redirect()->back()->with('success', 'Predicción actualizada exitosamente');
            } else {
                // Create new prediction
                $prediction = Prediction::create($validated);
                return redirect()->back()->with('success', 'Predicción guardada exitosamente');
            }

        } catch (\Exception $e) {
            return redirect()->back()->withErrors(['error' => 'Error al guardar la predicción: ' . $e->getMessage()]);
        }
    }
}

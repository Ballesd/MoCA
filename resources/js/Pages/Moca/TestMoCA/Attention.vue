    ...
    <div class="flex items-center justify-center space-x-12 mt-16 mb-10 w-full">
        <div class="w-7/12 flex flex-col gap-4">
            ...
            <div class="flex justify-start items-center space-x-3">
                ...
                <font-awesome-icon v-if="!countAleter && !sevenMinusSeven" :icon="['fas', 'volume-up']" size="2x" class="text-secondary cursor-pointer hover:text-primary" @click="speachIntroduction" />
                <font-awesome-icon v-if="countAleter && !sevenMinusSeven" :icon="['fas', 'volume-up']" size="2x" class="text-secondary cursor-pointer hover:text-primary" @click="countAleterSpeach" />
                <font-awesome-icon v-if="countAleter && sevenMinusSeven" :icon="['fas', 'volume-up']" size="2x" class="text-secondary cursor-pointer hover:text-primary" @click="audioSevenMinus" />
            </div>
            <div v-if="!orderNumber">
                <div class="border-2 border-gray-400 rounded-lg p-4 flex items-center">
                    <p class="text-gray-500 text-center">A continuación deberá completar 3 tareas, las cuales tendrán audio incorporado. Recuerde que solo tendrá una oportunidad para reproducirlos.</p>
                </div>
                <div class="w-full mt-4">
                    <ButtonCustom class="w-full" mode="button" :disabled="buttonState" @click="startNumberTest">INICIAR LAS PRUEBAS DE NÚMEROS</ButtonCustom>
                </div>
            </div>
            <!-- / Intro -->
            <div v-if="repeatNumeber">
                ...
                <div v-if="orderNumber && !inverseNumber" class="flex flex-col items-center gap-4 w-full">
                    ...
                    <p class="flex justify-center text-gray-500">Ingrese la primera serie de numeros en el mismo orden</p>
                    <div class="flex justify-center">
                        <div class="grid grid-cols-5 gap-7">
                            <div v-for="(number, index) in orderNumbers" :key="index">
                                <TextInput v-model="answerOrder[index]" ref="inputField" maxlength="1" class="h-12 w-12 text-center text-xl text-gray-600" />
                            </div>
                        </div>
                    </div>
                ...
                <div v-if="inverseNumber && orderNumber" class="flex flex-col items-center gap-4 w-full">
                    ...
                    <p class="flex justify-center text-gray-500">Ingrese la segunda serie de numeros en orden inverso</p>
                    <div class="flex justify-center">
                        <div class="grid grid-cols-3 gap-7">
                            <div v-for="(number, index) in InverseNumbers" :key="index">
                                <TextInput v-model="answerInverse[index]" ref="inputField" maxlength="1" class="h-12 w-12 text-center text-xl text-gray-600" />
                            </div>
                        </div>
                    </div>
                    <ButtonCustom class="w-1/3 mt-4" mode="button" @click="recordAttemptOrderInve">SIGUIENTE</ButtonCustom>
                </div>
                <!-- /Orden de numeros al inverso -->
            </div>
            ...
            <div v-if="sevenMinSeven" class="flex flex-col items-center gap-4 w-full">
                ...
                <p class="flex justify-center text-gray-500 text-center">Digite el número en el cuadro de texto y oprima la flecha de abajo para escribir el siguiente número.</p>
...
const countAleterSpeach = () => {
    const text1 = 'Ingrese la cantidad de golpes por la letra A elavorada por el paciente.';
    const synthesis = window.speechSynthesis;
    const utterance = new SpeechSynthesisUtterance(text1);
    utterance.rate = 0.7;
    synthesis.speak(utterance);
